...
## 🎓 Demonstrating to the Professor

1. **Live MongoDB Vector Search Inspector** ([http://localhost:3000/dashboard/llm-pipeline](http://localhost:3000/dashboard/llm-pipeline)):
   - View live MongoDB vector collection stats (768 dimensions, cosine similarity).
   - Run semantic vector queries with species pre-filtering.
   - Inspect the exact MongoDB `$vectorSearch` aggregation pipeline returned by the backend.

2. **RAG Natural Language Query Assistant** ([http://localhost:3000/dashboard/query](http://localhost:3000/dashboard/query)):
   - Submit queries (e.g. *"What are the withdrawal periods for common cattle antibiotics?"*).
   - Observe live pipeline execution, latency, token consumption, and evidence chunks retrieved from MongoDB.

3. **Live Veterinary NER Text Analyzer** ([http://localhost:3000/dashboard/ai-analytics](http://localhost:3000/dashboard/ai-analytics)):
   - Enter or edit any clinical text, select case presets, and execute real-time extraction via `POST /api/ner`.

4. **Live Data Explorer** ([http://localhost:3000/dashboard/data](http://localhost:3000/dashboard/data)):
   - Browse records loaded directly from `GET /api/data/records` with server-side filtering.
